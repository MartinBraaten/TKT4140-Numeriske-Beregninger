...
def heun(func, z0, xt):    
    ...
    for i, x in enumerate(xt[0:-1]):     
        dx = xt[i+1] - xt[i]        
        zp = z[i,:] + dx * np.asarray(func(z[i,:],x))
        z[i+1,:] = z[i,:] + dx/2*(np.asarray(func(z[i,:],x))+np.asarray(func(zp,x+dx))
    return z
#------------------------------------------------------------------
def dsfunction(phi0,phi1,s0,s1):    
    if (abs(phi1-phi0)>0.0):           
        ...
        return 0.0
#------------------------------------------------------------------
def f(z, t):    """RHS for hanging cable"""    
    zout    = np.zeros_like(z)    
    zout[:] = [z[1],(w/T)*np.sqrt(1+z[0]*z[0])]
    return zout 
#------------------------------------------------------------------# 
'Physical parameters'
#------------------------------------------------------------------# 
...
#------------------------------------------------------------------
legends=[] 
# empty list to append legends as plots are generated
plot(x,z[:,0])legends.append(r'$u(x)$')
## Add the labels
